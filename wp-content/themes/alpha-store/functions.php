...
if ( !function_exists( 'alpha_store_setup' ) ) :
	...
	function alpha_store_setup() {
		...
		add_image_size( 'alpha-store-carousel', 270, 423, true );
...
add_action( 'customize_controls_print_footer_scripts', 'alpha_store_pro_banner' );

function alpha_store_pro_banner() {
	echo '<a href="' . esc_url( 'http://themes4wp.com/product/alpha-store-pro/' ) . '" id="pro-banner" style="display: none; margin-top: 10px; background: #192429;" target="_blank"><img src="' . get_template_directory_uri() . '/img/alpha-store-pro.jpg" /></a>';
...
function alpha_store_theme_stylesheets() {
	...
	wp_enqueue_style( 'alpha-store-stylesheet', get_stylesheet_uri(), array(), '1.1.0', 'all' );
...
function alpha_store_breadcrumb() {
	...
	if ( is_home() || is_front_page() ) {
		...
		if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_for_posts' ) ) {
			echo '<span itemscope itemtype="' . esc_url( $schema_link ) . '"><a itemprop="url" href="' . esc_url( get_permalink( get_option( 'page_for_posts' ) ) ) . '">' . '<span itemprop="title">' . __( 'Blog', 'alpha-store' ) . '</span></a></span>' . $delimiter . ' ';
		}
		...
		if ( is_category() ) {
			...
			echo '<span itemscope itemtype="' . esc_url( $schema_link ) . '"><a itemprop="url" href="' . esc_url( get_permalink() ). '">' . '<span itemprop="title">' . ucfirst( $post_slug ) . '</span>' . '</a></span>';
		} elseif ( is_page() && $post->post_parent ) {
		...
		elseif ( is_tag() ) {
			...
			echo __( 'Error 404', 'alpha-store' );
		} elseif ( is_search() ) {
			echo __( 'Search results for', 'alpha-store' ) . ' ' . get_search_query();
		} elseif ( is_day() ) {
		...
		if ( get_query_var( 'paged' ) ) {
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() )
				echo ' (';
			echo __( 'Page', 'alpha-store' ) . ' ' . get_query_var( 'paged' );
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() )
				echo ')';
		}
...
if ( !function_exists( 'alpha_store_social_links' ) ) :
	...
	function alpha_store_social_links() {
		$twp_social_links	 = array( 'twp_social_facebook'	 => __( 'Facebook', 'alpha-store' ),
		...
		function alpha_store_head_wishlist() {
			if ( function_exists( 'YITH_WCWL' ) ) {
				...
				<div class="top-wishlist text-right">
					<a href="<?php echo esc_url( $wishlist_url ); ?>" title="Wishlist" data-toggle="tooltip" data-placement="top">
