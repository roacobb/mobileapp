...
add_action( 'widgets_init', 'liveblog_recent_posts_widget' );  

// Register Widget
function liveblog_recent_posts_widget() {
    register_widget( 'liveblog_recent_widget' );
}

// Widget Class
class liveblog_recent_widget extends WP_Widget {
     ...
     function __construct() {
        ...
        __('(ThemesPie) Recent Posts', 'liveblog'), 

        // Widget description
        array( 'description' => __('A widget that displays the recent posts of your blog', 'liveblog'), ) 
        );
    }
	...
	public function widget( $args, $instance ) {
		...
		$title = isset( $instance['title'] ) ? apply_filters('widget_title', $instance['title'] ) : __('Recent Posts', 'liveblog');
		...
		<ul class="recent-posts">
			<?php
                $recent_posts = new WP_Query('showposts='.$posts.'&orderby=date&order=DESC&ignore_sticky_posts=1');
	...
	public function form( $instance ) {
		...
		$defaults = array(
			'title' => __('Recent Posts', 'liveblog'),
			'posts' => 4,
			'show_thumb' => 1,
			'show_cat' => 0,
			'show_author' => 0,
