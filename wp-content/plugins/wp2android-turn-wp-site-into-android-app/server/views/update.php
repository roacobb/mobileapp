<?php
	...
	//wp2android remote update script
	class MLUpgrader {
    	...
    	$fd = fopen("../wp2android_last.zip", "w");
		...
		function download() {
			$download_file = $this->download_url("http://www.wp2android.wps.edu.vn/plugin/latest.zip");
		...
		function unpack() {
			$zip = new ZipArchive;
    	$res = $zip->open('../wp2android_last.zip');
			if ($res === TRUE) {
