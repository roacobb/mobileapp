...
if (!empty($action)) {
	...
	elseif ($action == 'basemap') {
		if ($_POST['marker-basemap-layer'] == 'all') {
			...
			$result = $wpdb->prepare( "UPDATE `$table_name_markers` SET `basemap` = %s WHERE `layer` LIKE %s", $_POST['basemap'], '%"'. intval($_POST['marker-basemap-layer']) .'"%' );
		}
		$wpdb->query( $result );
		$wpdb->query( "OPTIMIZE TABLE `$table_name_markers`" );
		echo '<p><div class="notice notice-success is-dismissible" style="padding:10px;">' . sprintf( esc_attr__('The basemap for the selected markers has been successfully set to %1$s','lmm'), htmlspecialchars($_POST['basemap'])) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
	}
	...
	elseif ($action == 'mapsize') {
		if ($_POST['marker-mapsize-layer'] == 'all') {
			...
			$result = $wpdb->prepare( "UPDATE `$table_name_markers` SET `mapwidth` = %d, `mapwidthunit` = %s, `mapheight` = %d WHERE `layer` LIKE %s", $_POST['mapwidth'], $_POST['mapwidthunit'], $_POST['mapheight'], '%"'. intval($_POST['marker-mapsize-layer']) .'"%' );
		}
		$wpdb->query( $result );
		$wpdb->query( "OPTIMIZE TABLE `$table_name_markers`" );
		echo '<p><div class="notice notice-success is-dismissible" style="padding:10px;">' . sprintf( esc_attr__('The map size for all markers has been successfully set to width =  %1$s %2$s and height = %3$s px','lmm'), htmlspecialchars($_POST['mapwidth']), htmlspecialchars($_POST['mapwidthunit']), htmlspecialchars($_POST['mapheight'])) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
	}
	elseif ($action == 'zoom') {
		...
		$wpdb->query( $result );
		$wpdb->query( "OPTIMIZE TABLE `$table_name_markers`" );
		echo '<p><div class="notice notice-success is-dismissible" style="padding:10px;">' . sprintf( esc_attr__('Zoom level for the selected markers has been successfully set to %1$s','lmm'), htmlspecialchars($_POST['zoom'])) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
	}
	...
	elseif ($action == 'icon') {
		if ($_POST['marker-icon-layer'] == 'all') {
			...
			$result = $wpdb->prepare( "UPDATE `$table_name_markers` SET `icon` = %s WHERE `layer` LIKE %s", $_POST['icon'], '%"'. intval($_POST['marker-icon-layer']).'"%' );
		}
		$wpdb->query( $result );
		$wpdb->query( "OPTIMIZE TABLE `$table_name_markers`" );
	...
	elseif ($action == 'gpx-url') {
		if ( (lmm_isValidURL( $_POST['marker-gpx-url'] )) || ($_POST['marker-gpx-url'] == NULL) ) {
			if ($_POST['marker-gpx-url-layer'] == 'all') {
				$result = $wpdb->prepare( "UPDATE `$table_name_markers` SET `gpx_url` = %s", trim($_POST['marker-gpx-url']) );
			} else {
				$result = $wpdb->prepare( "UPDATE `$table_name_markers` SET `gpx_url` = %d WHERE `layer` LIKE %s", trim($_POST['marker-gpx-url']), '%"'.intval($_POST['marker-gpx-url-layer']).'"%' );
			}
			$wpdb->query( $result );
			$wpdb->query( "OPTIMIZE TABLE `$table_name_markers`" );
			...
			echo '<p><div class="notice notice-error" style="padding:10px;">' . __('The URL to GPX has not been updated - URL to GPX track does not seem to be a valid url!','lmm') . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
		}
	}
	...
	elseif ($action == 'basemap-layer') {
		...
		echo '<p><div class="notice notice-success is-dismissible" style="padding:10px;">' . sprintf( esc_attr__('The basemap for all layers has been successfully set to %1$s','lmm'), htmlspecialchars($_POST['basemap-layer'])) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
	}
	...
	elseif ($action == 'mapsize-layer') {
		...
		$wpdb->query( $result );
		$wpdb->query( "OPTIMIZE TABLE `$table_name_layers`" );
		echo '<p><div class="notice notice-success is-dismissible" style="padding:10px;">' . sprintf( esc_attr__('The map size for all layers has been successfully set to width =  %1$s %2$s and height = %3$s px','lmm'), htmlspecialchars($_POST['mapwidth-layer']), htmlspecialchars($_POST['mapwidthunit-layer']), htmlspecialchars($_POST['mapheight-layer'])) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
	}
	elseif ($action == 'zoom-layer') {
		$result = $wpdb->prepare( "UPDATE `$table_name_layers` SET `layerzoom` = %s", $_POST['zoom-layer'] );
		$wpdb->query( $result );
		$wpdb->query( "OPTIMIZE TABLE `$table_name_layers`" );
		echo '<p><div class="notice notice-success is-dismissible" style="padding:10px;">' . sprintf( esc_attr__('Zoom level for all layers has been successfully set to %1$s','lmm'), htmlspecialchars($_POST['zoom-layer'])) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
	}
	...
	elseif ($action == 'change_marker_id') {
		...
		if ($old_marker_id_exists == NULL) {
			echo '<p><div class="notice notice-error" style="padding:10px;">' . sprintf( esc_attr__('Error: a marker with the ID %1$s does not exist!','lmm'), $_POST['marker_id_old']) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
		} else {
			...
			if ($new_marker_id_exists != NULL) {
				echo '<p><div class="notice notice-error" style="padding:10px;">' . sprintf( esc_attr__('Error: a marker with the ID %1$s already exists!','lmm'), $_POST['marker_id_new']) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
			} else {
	...
	elseif ($action == 'change_layer_id') {
		...
		if ($old_layer_id_exists == NULL) {
			echo '<p><div class="notice notice-error" style="padding:10px;">' . sprintf( esc_attr__('Error: a layer with the ID %1$s does not exist!','lmm'), $_POST['layer_id_old']) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
		} else {
			...
			if ($new_layer_id_exists != NULL) {
				echo '<p><div class="notice notice-error" style="padding:10px;">' . sprintf( esc_attr__('Error: a layer with the ID %1$s already exists!','lmm'), $_POST['layer_id_new']) . '</div><br/><a class="button-secondary" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools">' . __('Back to Tools', 'lmm') . '</a></p>';
			} else {
	...
	elseif ($action == 'database_upgrade') {
							...
							<td><?php _e('Assign the following layer instead','lmm'); ?>
								...
								<?php
									foreach ($layerlist as $row) {
										...
										echo '<option value="' . $row['id'] . '">' . mb_substr(stripslashes(htmlspecialchars($row['name'])), 0, 28) . $layername_abstract . ' (ID ' . $row['id'] . ')</option>';
									}
								?>
								</select>
							</td>
		...
		<table class="widefat fixed" style="width:auto; margin-top:10px;">
			<?php
				...
				<?php
					...
					if ( version_compare( $wp_version, '3.9-alpha', '>=' ) ) {
						function lmm_plugin_mce_css( $mce_css ) {
							...
							$custom_tinymce_css = LEAFLET_PLUGIN_URL . 'inc/css/leafletmapsmarker-admin-tinymce.php?nonce=' . $tinymce_custom_css_nonce . '&defaults_marker_popups_maxwidth=' . $defaults_marker_popups_maxwidth . '&defaults_marker_popups_image_css=' . $defaults_marker_popups_image_css . '&wordpress_version='.$wp_version.'&timestamp='.time();
							if ( ! empty( $mce_css ) ) {
								$mce_css .= ',';
							}
