...
class MMP_RESTAPI{
	...
	public function add_endpoint(){
		add_rewrite_endpoint( 'mmp-api', EP_ALL );
	}
	/**
	...
	public function process_query(){
		...
		// Check for mmp-api var. Get out if not present
		if ( empty( $wp_query->query_vars['mmp-api'] ) ) {
			return;
		}
	...
	private function get_api_query(){
		...
		$query = isset( $wp_query->query_vars['mmp-api'] ) ? $wp_query->query_vars['mmp-api'] : null;
		if(!is_null($query)){
			$query = explode('/', $query);
	...
	public function output( $status_code = 200 ) {
		...
		switch ( $format ) :
			...
			case 'xml' :
				require_once LEAFLET_PLUGIN_DIR . 'inc/array2xml.php';
...
$_GLOBALS['mmp-restapi'] =	new MMP_RESTAPI();