	...
	if ($action_standalone == NULL) {
		...
		if ($action_iframe == 'import') {
			...
			**********************************/
			...
			echo '<td><h1 style="font-size:20px;margin:0px;"> ' . __('prepare import','lmm') . ' (' . __('markers','lmm') . ')</h1></td></tr></table>';
			echo '
			<script>
				...
				<tr>
					<td colspan="2">
						...
						<ul>
							...
							if ($lmm_options['google_maps_api_key'] == NULL) {
								echo '<li><strong>' . sprintf(__('If you want to use geocoding via Google Places API, you need to <a href="%1$s" target="_top">configure your API key</a> first!','lmm'), LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_settings#lmm-google') . '</strong></li>';
							}
					echo '</ul>
					</td>
				</tr>
		...
		//info: end ($action_iframe == 'import-layers')
			...
			**********************************/
			...
			echo '<td><h1 style="font-size:20px;margin:0px;"> ' . __('prepare export','lmm') . ' (' . __('markers','lmm') . ')</h1></td></tr></table>';
					...
					<td>
						...
						foreach ($iconlist as $row) {
							if ($row['icon'] == NULL) {
								...
								echo '<div style="text-align:center;float:left;line-height:0px;margin-bottom:3px;"><label for="'.$row['icon'].'"><img src="' . $defaults_marker_icon_url . '/' . $row['icon'] . '" title="' . esc_attr__('filename','lmm') . ': ' . $row['icon'] . '" alt="' . $row['icon'] . '" width="' . intval($lmm_options[ 'defaults_marker_icon_iconsize_x' ]) . '" height="' . intval($lmm_options[ 'defaults_marker_icon_iconsize_y' ]) . '" /></label><br/><input id="'.$row['icon'].'" style="margin:1px 0 0 1px;" type="radio" name="filter-icon" value="'.$row['icon'].'"/></div>';
							}
						}
					echo '</div>
					</td>
							...
							if ( $objWorksheet->getCellByColumnAndRow(6, $row)->getDataType() == 'n' ) {
								...
								if ($geocoding_option == 'geocoding-on') {
									...
									if (strpos($objWorksheet->getCellByColumnAndRow(6, $row), '.') === FALSE) {
										...
										echo date('H:i:s') . ' <a name="' . $row . '"></a><span class="warning">' . sprintf(__('Warning: invalid value for %1$s in row %2$s (%3$s): please use , and not . as comma separator! Using default value %4$s instead','lmm'), 'lon', $row, $objWorksheet->getCellByColumnAndRow(6, $row), str_replace(".", ",", floatval($lmm_options[ 'defaults_marker_lon' ])));
									}
									echo '</span><br/>';
									$stats_warnings[] = $row;
									$lon = floatval($lmm_options[ 'defaults_marker_lon' ]);
						...
						if ($import_mode == 'import-mode-add') {
							...
							if ( $objWorksheet->getCellByColumnAndRow(4, $row)->getDataType() == 'n' ) {
								...
								if ($geocoding_option == 'geocoding-on') {
									...
									if (strpos($objWorksheet->getCellByColumnAndRow(4, $row), '.') === FALSE) {
										...
										echo date('H:i:s') . ' <a name="' . $row . '"></a><span class="warning">' . sprintf(__('Warning: invalid value for %1$s in row %2$s (%3$s): please use , and not . as comma separator! Using default value %4$s instead','lmm'), 'layerviewlon', $row, $objWorksheet->getCellByColumnAndRow(4, $row), str_replace(".", ",", floatval($lmm_options[ 'defaults_layer_lon' ])));
									}
									echo '</span><br/>';
									$stats_warnings[] = $row;
									$layerviewlon = floatval($lmm_options[ 'defaults_layer_lon' ]);
							...
							if ($objWorksheet->getCellByColumnAndRow(36, $row)->getDataType() == 'n') {
								if ( ($objWorksheet->getCellByColumnAndRow(36, $row) == '0') || ($objWorksheet->getCellByColumnAndRow(36, $row) == '1') ) {
									...
									echo date('H:i:s') . ' <a name="' . $row . '"></a><span class="warning">' . sprintf(__('Warning: invalid value for %1$s in row %2$s (%3$s) - using default value %4$s instead','lmm'), 'mlm_filter', $row, $objWorksheet->getCellByColumnAndRow(36, $row), '0') . '</span><br/>';
									$stats_warnings[] = $row;
									$mlm_filter = '0';
								}
							} else {
							...
							if ($objWorksheet->getCellByColumnAndRow(37, $row)->getDataType() != 'null') {
								...
								if (json_decode($objWorksheet->getCellByColumnAndRow(37, $row)) != NULL ) {
									...
									echo date('H:i:s') . ' <a name="' . $row . '"></a><span class="warning">' . sprintf(__('Warning: invalid value for %1$s in row %2$s (%3$s) - using default value %4$s instead','lmm'), 'mlm_filter_details', $row, $objWorksheet->getCellByColumnAndRow(37, $row), 'NULL') . '</span><br/>';
									$stats_warnings[] = $row;
									$mlm_filter_details = '';
								}
							} else {
							...
							if ($existing_layer_data['id'] != NULL) {
								...
								if ($objWorksheet->getCellByColumnAndRow(36, $row)->getDataType() == 'n') {
									if ( ($objWorksheet->getCellByColumnAndRow(36, $row) == '0') || ($objWorksheet->getCellByColumnAndRow(36, $row) == '1') ) {
										...
										echo date('H:i:s') . ' <a name="' . $row . '"></a><span class="warning">' . sprintf(__('Warning: invalid value for %1$s in row %2$s (%3$s) - using current value %4$s instead','lmm'), 'mlm_filter', $row, $objWorksheet->getCellByColumnAndRow(36, $row), $existing_layer_data['mlm_filter']) . '</span><br/>';
										$stats_warnings[] = $row;
								...
								if ($objWorksheet->getCellByColumnAndRow(37, $row)->getDataType() != 'null') {
									...
									if (json_decode($objWorksheet->getCellByColumnAndRow(37, $row)) != NULL ) {
										...
										echo date('H:i:s') . ' <a name="' . $row . '"></a><span class="warning">' . sprintf(__('Warning: invalid value for %1$s in row %2$s (%3$s) - using current value %4$s instead','lmm'), 'mlm_filter_details', $row, $objWorksheet->getCellByColumnAndRow(37, $row), $existing_layer_data['mlm_filter_details']) . '</span><br/>';
										$stats_warnings[] = $row;
		...
		//info: end (action_standalone == export)
			...
			**********************************/
			...
			while ($array_count_current < $array_count_total) {
				...
				$objPHPExcel->getActiveSheet()->setCellValue('AK'.$rowNumber,$export_rows[$array_count_current]['mlm_filter']);
				$objPHPExcel->getActiveSheet()->setCellValue('AL'.$rowNumber,$export_rows[$array_count_current]['mlm_filter_details']);
				$rowNumber++;
				$array_count_current++;
			}
