	...
	foreach($array as $one):
		...
		if ($one['type'] == 'marker') {
			...
			if ($one['icon'] == NULL) {
				...
				$list_entry = '<img src="' . LEAFLET_PLUGIN_ICONS_URL . '/' . $one['icon'] . '" width="' . intval($lmm_options[ 'defaults_marker_icon_iconsize_x' ]) . '" height="' . intval($lmm_options[ 'defaults_marker_icon_iconsize_y' ]) . '" title="' . $title . '"/>';
			}
		} else {
