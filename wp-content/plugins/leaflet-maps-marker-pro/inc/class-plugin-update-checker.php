...
class PluginUpdateChecker_2_2 {
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			...
			//info: retry for server with outdated ssl config
			$result = wp_remote_get(
				'http://' . $url,
				$options
			);
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, 'Manually check for updates');
			if ( !empty($linkText) ) {
...
//RH PucFactory::addVersion('PucGitHubChecker', 'PucGitHubChecker_2_2', '2.2');